        ...
        if (process.env.NODE_ENV === 'development') {
            switch (name) {
                ...
                case 'DATABASE_URL':
                    ...
                    break;
                case 'OPENAI_API_KEY':
                    console.warn('WARNING: OPENAI_API_KEY not set, quotation features may not work');
                    break;
            }
        }
...
const fromEnv = () => {
    try {
        ...
        console.log('- DATABASE_URL:', environment.databaseUrl.replace(/\/\/([^:]+):([^@]+)@/, '//***:***@'));
        console.log('- OPENAI_API_KEY:', environment.openaiApiKey ? '[Set]' : '[Not Set]');
        return environment;
    }
    catch (error) {
