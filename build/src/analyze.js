...
async function analyzeMood(quote) {
    try {
        ...
        if (response.data?.choices?.[0]?.message?.content) {
            ...
            const normalizedMood = AVAILABLE_MOODS.find((m) => m.toLowerCase() === mood.toLowerCase());
            if (!normalizedMood) {
                console.warn(`OpenAI returned invalid mood: ${mood}. Using default mood: Reflective`);
...
async function saveQuoteToDatabase(quote, mood) {
    try {
        console.log(`Saving quote to database with mood: ${mood}`);
        const affirmation = await prisma_1.prisma.affirmation.create({
            data: {
                content: quote,
                mood_type: mood,
            },
