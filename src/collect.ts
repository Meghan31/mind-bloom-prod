...
async function getQuoteFromOpenAI(): Promise<string> {
	try {
			...
			messages: [
				{
					role: 'system',
					content:
						'You are a helpful assistant that generates inspirational and uplifting quotes.',
				},
				{
					role: 'user',
    ...
    ${newMood}
      ...
      Make it positive and non-cliché. Only include the quote text, and don't start the quote with embrace, be, or live., make a unique quote.`,
		...
		if (axios.isAxiosError(error)) {
			console.error(
				'Error getting quote from OpenAI:',
...
function saveQuote(quote: string): void {
	try {
		...
		try {
			...
			if (!Array.isArray(quotes)) {
				quotes = [];
				console.warn(
					'Quotes file did not contain an array, initializing new array'
				);
			}
		} catch (e) {
